...
def generate_games(n: int, start_date: str, words: List[str]):
  ...
  while len(games) < n:
    ...
    while len(solved_swaps["order"]) <= 3:
      # generate and validate indexes to swap
      rand_indexes = []
      while len(rand_indexes) < 2:      
        tmp = indexes[random.randint(0, len(indexes)-1)]
        if get_cell(tmp[0], tmp[1]) not in locks:
      ...
      # record the swap
      i_cell = get_cell(i[0], i[1])
      j_cell = get_cell(j[0], j[1])
      solved_swaps["order"].append({
        "i": i_cell,
        "j": j_cell,
      })

      if i_cell in solved_swaps["cellInfo"]:
        solved_swaps["cellInfo"][i_cell][j_cell] = class_map[len(solved_swaps["order"])]
      else:
        solved_swaps["cellInfo"][i_cell] = { j_cell: class_map[len(solved_swaps["order"])] }

      if j_cell in solved_swaps["cellInfo"]:
        solved_swaps["cellInfo"][j_cell][i_cell] = class_map[len(solved_swaps["order"])]
      else:
        solved_swaps["cellInfo"][j_cell] = { i_cell: class_map[len(solved_swaps["order"])] }
...
if __name__ == "__main__":
  ...
  games = generate_games(int(args.n), args.start_date, wordies)
