<script lang="ts">
  ...
  function handleUndo(whichGuesses: string | undefined) {
	...
	const handleEnter = () => {
    console.log('Enter')
    let topTarget = (topGuesses.val.length ? topGuesses.val[topGuesses.val.length-1] : todaysGame.topWord).toUpperCase()
    let bottomTarget = (bottomGuesses.val.length ? bottomGuesses.val[0] : todaysGame.bottomWord).toUpperCase()
    let matchesTop = hammingDistance(guessInput.val, topTarget) === goalDistance
    let matchesBottom = hammingDistance(guessInput.val, bottomTarget) === goalDistance
