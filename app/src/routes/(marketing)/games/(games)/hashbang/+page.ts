...
export const load: PageLoad = async ({ fetch }) => {
  const resDate = await fetch(`https://s3.amazonaws.com/jamcatwow.com.games/hashbang/days/${todaysDateIso}.json`);
  const game: Game = await resDate.json();
  
  const resYesterdaysDate = await fetch(`https://s3.amazonaws.com/jamcatwow.com.games/hashbang/days/${yesterdaysDateIso}.json`);
